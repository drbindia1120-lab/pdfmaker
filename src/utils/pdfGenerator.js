...
export const generateDealsPDF = async (products) => {
  ...
  try {
    const imgData = await fetch('/ROTTO+.png')
      .then(res => res.blob())
  ...
  const inquiryText = 'Inquiry: +91 9316019284';
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);

  const pillPadding = 16; // padding for premium look
  const pillW_inquiry = doc.getTextWidth(inquiryText) + pillPadding;
  const pillH = 10;

  const startX = (pageWidth / 2) - (pillW_inquiry / 2);
  ...
  // Draw Inquiry Pill Button
  doc.setFillColor(250, 250, 250); // Explicitly set background color
  doc.roundedRect(startX, pillY, pillW_inquiry, pillH, 5, 5, 'FD');
  doc.setTextColor(...textColor);
  doc.text(inquiryText, startX + (pillW_inquiry / 2), pillY + 6.8, { align: 'center' });
  doc.link(startX, pillY, pillW_inquiry, pillH, { url: 'tel:+919316019284' }); // Click to call

  // Address
  doc.setTextColor(...textMuted);
  doc.setFontSize(12);
  doc.text('B-1, Bhagwati Estate Park, Opp. Ganga Heights,', pageWidth / 2, footerY + 22, { align: 'center' });
  doc.text('Moti Ved, Katargam, Surat, Gujarat - 395004', pageWidth / 2, footerY + 29, { align: 'center' });
  ...
  doc.link(btnX, btnY, btnW, btnH, { url: 'https://maps.app.goo.gl/E2pjUwobUAkzgcLx8?g_st=aw' });
